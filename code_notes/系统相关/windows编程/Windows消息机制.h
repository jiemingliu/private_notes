windows中消息虽然有很多，分为系统消息和用户自定义消息
Windows系统消息有三种：
...
添加消息处理函数的步骤
1.在类定义MainFrm.h中加入消息处理函数的函数声明，必须以afx_msg打头，
2.在类的消息映射表中添加该消息的消息映射入口项，在MainFrm.cpp的BEGIN_MESSAGE_MAP和END_MESSAGE_MAP之间
3.在类的实现MainFrm.cpp中添加消息处理函数的实现