...
struct DLL_EXPORT JCXLSFormatManager
{
	JCXLSFormatManager(JCBasicExcel& xls);
...
struct CellFormat
{
	CellFormat(JCXLSFormatManager& mgr)
	 :	_mgr(mgr)
	{
		init();
	}

	CellFormat(JCXLSFormatManager& mgr, const ExcelFont& font)
	 :	_mgr(mgr)
	{
		init();
		set_font(font);
	...
	CellFormat(JCXLSFormatManager& mgr, const Workbook::XF& xf, int xf_idx)
	 :	_mgr(mgr)
	{
		init(xf, xf_idx);
	}

	CellFormat(JCXLSFormatManager& mgr, const JCBasicExcelCell* cell)
	 :	_mgr(mgr)
	{
	...
	JCXLSFormatManager& get_format_manager() const
	{
		return _mgr;
	}
...
inline void JCBasicExcelCell::SetFormat(const CellFormat& fmt)
{
